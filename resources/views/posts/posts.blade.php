...
<head>
     ...
     <title>Posts - Rating System</title>
</head>
          ...
          @foreach ($posts as $post)
               <div class="post-container">
                    ...
                    <div class="comment-btns">
                         <img class="comment-btn" src="{{asset('SVG/comment.svg')}}" />
                    </div>
               </div>
               <hr>
               <div class="comments-container">
                    <div class="comments-box">
                         <img src="{{asset('SVG/close.svg')}}" class="close-btn" />
                         <div class="comment">
                              @foreach ($post['comments'] as $comment)
                              <div class="box">
                                   <div class="user-info">
                                        <img src="{{url('')}}/{{$comment->user->image}}" class="user-image" />
                                        <span>{{$comment->user->full_name}}</span>
                                   </div>
                                   <div class="text">{{$comment->comment}}</div>
                                   @if($comment->user_id == $user->id || $user->account_type == 'admin')
                                        <a href="{{url('/comments/'.$comment->id)}}" class="delete-btn">Delete</a>
                                   @endif
                                   <hr />
                              </div>
                              @endforeach
                         ...
                         @if ($user->account_type == 'regular user')
                              <form method="POST" action="{{url('/comments/'. $post->id)}}">
                                   ...
                                   <button type="submit"><img src="{{asset('SVG/send.svg')}}" /></button>
                              </form>
                         @endif
                    </div>
               </div>
